...
[AddComponentMenu("NGUI/Interaction/Button Message 2")]
public class UIButtonMessage2 : MonoBehaviour
{
	...
	private void Send()
	{
		...
		if (!string.IsNullOrEmpty(EnableComponent))
		{
			MonoBehaviour monoBehaviour = target.GetComponent(EnableComponent) as MonoBehaviour;
		...
		if (includeChildren)
		{
			...
			{
				transform.gameObject.SendMessage(functionName, SendMessageOptions.DontRequireReceiver);
			}
		}
		else
		{
